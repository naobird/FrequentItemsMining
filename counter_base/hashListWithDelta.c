...
void Output(HashList* hash_list, int threshold) {
    ...
    while (1) {
        ...
        while (1) {
            ...
            if (bucket->count - target->delta > threshold) {
                printf("%d %d\n", target->item, bucket->count - target->delta);
                number++;
            }
...
// set item_node again
void setItemNode(ItemNode* target, val item, int delta, Bucket* parent, ItemNode* prev_scl, ItemNode* next_scl, ItemNode* prev_shvl, ItemNode* next_shvl) {
    target->item = item;
    target->delta = delta;
...
void addItemNode(HashList* hash_list, val item, int hash_value, int add_count) {
    ...
    setItemNode(fetch_itemnode, item, add_count - 1, NULL, NULL, NULL, NULL, NULL);

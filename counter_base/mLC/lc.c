...
// decrement all items 
void DeleteItems (HashList* hash_list, int delete_times, int size) {
    if (hash_list->bucket->count == delete_times) {
        ...
        while (1) {
            ...
            //setItemNode(target_itemnode, 0, NULL, target_itemnode->prev_scl, target_itemnode->next_scl, NULL, NULL);
            // 上の一文(ItemNodeのポインタの処理) 後で使うならその時に書き換えるのでいらない
